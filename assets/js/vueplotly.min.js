...
(function processPlotlyData() {
  ...
  , plotlyFunctions = ["restyle", "relayout", "update", "addTraces", "deleteTraces", "moveTraces", "extendTraces", "prependTraces", "purge", "animate"];
  function cached(e) {
  ...
  Vue.component("plotly", {
    ...
    mounted() {
        if (this.keepselection) this.$el.classList.add("keep_selection")
        Plotly.newPlot(this.$el, this.data, this.innerLayout, this.config)
        if (this.frames && this.frames.length) {
            Plotly.addFrames(this.$el, this.frames)
        }
        events.forEach((e=>{
    ...
    watch: {
        ...
        frames: {
            handler(e) {
                if (e && e.length) {
                    const n = this.$el._transitionData?._frames?.length ?? 0
                    if (n > 0) { Plotly.deleteFrames(this.$el, [...Array(n).keys()]) }
                    Plotly.addFrames(this.$el, e)
                }
            },
            deep: true,
            immediate: false
